...
const ProductReview = ({ productData, packageId }) => {
  // Image upload state
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!packageId || !rating || !review || !createdBy) {
  ...
  const fetchReviews = async () => {
    ...
    try {
      const res = await fetch(`/api/productReviews?packageId=${packageId}`);
      const data = await res.json();
  ...
  const confirmDelete = async () => {
    ...
    try {
      const res = await fetch('/api/productReviews', {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ reviewId: deleteTargetId, packageId })
      });
      const data = await res.json();
                  ...
                  <Table className="min-w-full divide-y divide-gray-200">
                    <TableHeader>
                      <TableRow className="bg-gray-100">
                        ...
                        <TableHead className="px-4 py-3 text-center">Package Name</TableHead>
